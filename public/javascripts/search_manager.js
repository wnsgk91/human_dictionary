...
function search_wizard_home() {
  let input = JSON.parse(localStorage.getItem('search_history')); // 검색 히스토리 가져오기
  //filter = input[-1].value.toUpperCase(); // 대문자로 바뀐 input 값(검색어) 가져오기
  document.getElementById('search_search').value = input[-1]; // 검색 히스토리의 제일 마지막 값을 전달하기.
...
function search_wizard() {
  let input, filter, dict_list, li_tag, a_tag, i;
