...
function history_process(history_list){
  ...
  for (let i = five_items.length-1; i >= 0; i--){
    if(five_items[i] !== ''){
      history_banner += '<li class="list-group-item" ><div id="history_name"><a href="/search?keyword='+ five_items[i] +'">' + five_items[i] + '</a></div><div id="history_delete_btn" onclick="delete_select_history('+i+');">&times;</div></li>';
    }
    console.log(i);
  }
